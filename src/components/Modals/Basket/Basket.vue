    ...
    >
      ...
      <div class="basket-footer">
        <div
          ...
          v-show="basket.state.inBasket.length > 0"
        >
          <div class="text-white text-center text-[17px] p-3">
            {{ $t("basket.total") }}:
          </div>
        ...
        <div
          v-show="basket.state.inBasket.length === 0"
          class="text-white text-center text-[17px] p-3"
        >
          {{ $t("basket.empty") }}
        </div>
        <div
          v-show="basket.state.inBasket.length > 0"
          class="flex justify-around flex-wrap md:flex-nowrap gap-4 mb-3"
        >
          <button class="basket-btn" @click="clearBasket">
            {{ $t("basket.clearBasket") }}
          </button>
          <button class="basket-btn" @click="createOrder">
            {{ $t("basket.goToOrder") }}
          </button>
        </div>
      </div>
