<template>
  <MyModal
    :styles="'flex flex-col h-full justify-between max-h-[900px] max-w-[884px] min-h-[420px] modal p-5 w-full items-center'"
    :show="openProductDescription"
  ...
  >
    ...
    <div class="flex gap-2 w-full justify-evenly">
      ...
      >
        <span class="" v-if="checkInBasket(product?.uid)">{{
          $t("basket.productAdded")
        }}</span>
...
const pushToBasket = (uid) => {
  ...
  const existingProductIndex = basket.state.selectedProducts.findIndex(
    (product) => product.uid === uid,
  );
  const inBasket = basket.state.inBasket.findIndex(
    (product) => product === uid,
  );
  if (existingProductIndex !== -1) {
    basket.state.selectedProducts[existingProductIndex] = {
      ...props.product,
      quantity,
    };
  } else {
