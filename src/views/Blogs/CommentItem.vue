<template>
  <div v-for="(comment,idx) in comments" :key="comment" class="flex flex-col gap-3">
      ...
      <div class="flex flex-col gap-2 w-full">
        ...
        <div class="flex gap-2 justify-between">
          <div class="flex gap-2">
            <img src="../../assets/images/svg/like-icon.svg" class="cursor-pointer hover:scale-110 transition duration-300 ease-in-out" @click="updateLike(idx, props.commentId)">
    ...
    <Transition name="slide-fade">
      <CommentBtnGroup
        ...
        @save-comment="(textComment)=>{ onReplyComment(textComment, idx, props.commentId, comment.author?.displayName) }"
        @cancel-action="onCancelAction"
        :placeholder="$t('blog.addComment')"
      >
        <template v-slot:cancel-btn>{{$t('reused.cancel')}}</template>
