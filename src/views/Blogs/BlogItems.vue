<template>
  <div
    ...
    <div class="flex items-center gap-5 cursor-pointer transition duration-700 ease-in-out bg-white rounded"
         v-for="(item,index) in blogStore.state.data" :key="item" @click="openModal(index)">
      <img :src="item.imgUrl" class="w-full max-w-[200px] max-h-[200px] object-cover" alt="">
      <div class="flex flex-col w-full h-full gap-2">
        ...
        <div class="py-3 flex flex-col gap-6 text-custom-gray text-sm pe-3">
          <div class="flex gap-2">
            ...
            <span>{{ moment(item.timeCreate).format('LLLL') }}</span>
          </div>
